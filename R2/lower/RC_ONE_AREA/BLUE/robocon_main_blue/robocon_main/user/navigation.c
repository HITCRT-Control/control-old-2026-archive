...
void Point_to_Point(PATH_POINT *p)
{
	...
	else if (p->point_inx == p->point_num - 1)
	{
		...
		switch(nav_reach_state)
		{
			...
			case 7: //一区对接，不需要很严格，因为结束后会继续锁位置微调
	...
	if(p->point_inx == p->point_num - 1){
			...
			{
						K_V = 0.55*(0.6f + 0.4f * cosf(PI * progress));
			}
	...
	else if(nav.auto_path.number_point!=7&&nav.auto_path.number_point!=8&&nav.auto_path.number_point!=9&&nav.auto_path.number_point!=32&&nav.auto_path.number_point!=35)
	{
			K_V=0.85f*(0.9f + 0.1f * cosf(PI * progress));
			K_VY=0.9*(0.9f + 0.1f * cosf(PI * progress));
			K_VW=0.9;
	}
	else
	{
