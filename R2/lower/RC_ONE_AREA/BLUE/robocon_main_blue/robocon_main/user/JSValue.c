...
void check_fps(void)
{
	...
	if(monitor.rate_fps.vision<=200){monitor_error.vision=0;}else {monitor_error.vision=1;}		
...
void remote_send_(void)
{
	...
	remote_send[18] = 1;    //代表红场或者蓝场  蓝为1 红为2
	remote_send[19] = 3;    //代表三个赛项  1为九宫藏宝 2为崇武探幽 3为正赛
	remote_send[20] = 0;
	remote_send[21] = 0;
	remote_send[22] = 0;
	remote_send[23] = 0;

	uint16_t dt35_x = (uint16_t)DT35_X_fact;
	uint16_t dt35_y = (uint16_t)DT35_Y_fact;
	memcpy(&remote_send[24], &dt35_x, 2);
	memcpy(&remote_send[26], &dt35_y, 2);
