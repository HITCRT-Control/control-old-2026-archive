...
void ramp_task(void)
{
    ...
    case 0: // 根据出口走第一段路径
        ...
        if (nav.nav_state == NAV_RAMP)
        {
						...
						nav.auto_path.pos_pid.pid_w.fpKp = 2.7f;
						nav.auto_path.pos_pid.pid_x.fpKp = 1.7f;nav.auto_path.pos_pid.pid_y.fpKp =1.7f;
    ...
    case 1: // 根据第一段结束的速度走贝塞尔曲线到达台阶前，同时预加载
        ...
        nav.expect_robot_global_velt.fpX = 0.85*Velt_Ramp.vx + nav.auto_path.pos_pid.pid_x.fpU ;
    ...
    case 2: // 恒功率控制上坡，其中可加动态重心补偿
        ...
        nav.expect_robot_global_velt.fpY = -0.95*RAMP_CRUISE_SPEED +nav.auto_path.pos_pid.pid_y.fpU ;
...
void Ramp_test(void)
{
    ...
    static uint8_t prev_state = -1;
    static uint32_t time_ramp=0;
    if (ramp_test!= prev_state)/* 状态变化，重置标志 */
    {
        prev_state = ramp_test;
        nav_inited = 0;
        time_ramp=0;
    }
    switch (ramp_test)
    ...
    case 0: // 根据出口走第一段路径
        ...
        if (nav.nav_state == NAV_RAMP)
        {
					nav.auto_path.pos_pid.pid_w.fpKp=4.0f;
					nav_reach_state = 1;
            ramp_test = 1;
        }
        break;
    case 1: // 根据第一段结束的速度走贝塞尔曲线
        if (!nav_inited)
        {
            P0.x = nav.auto_path.pos_pid.pid_x.fpFB; // 根据实际XY再做改变
            P0.y = nav.auto_path.pos_pid.pid_y.fpFB;
            P2.x = 258.040741; // 记得测！！！
            P2.y = -9366.f;
            P1.x = P2.x;//166.f
            P1.y = P0.y;//-10224.f
            ...
            ramp_speed=800.f;
            Bezier_init(&bezier_use, P0, P1, P2, ramp_speed);
            nav_inited = 1;
            time_ramp = 0;
						nav.auto_path.pos_pid.pid_w.fpDes = nav.auto_path.pos_pid.pid_w.fpFB;
        }
        Bezier_calc_by_time(&bezier_use, time_ramp, &Point_Ramp, &Velt_Ramp); // 计算每一时刻理论位置速度
        time_ramp++;//ms
        nav.auto_path.pos_pid.pid_x.fpDes = Point_Ramp.x;
        ...
        PID_Calc(&nav.auto_path.pos_pid.pid_x, nav.auto_path.pos_pid.pid_x.fpDes, nav.auto_path.pos_pid.pid_x.fpFB);
        PID_Calc(&nav.auto_path.pos_pid.pid_y, nav.auto_path.pos_pid.pid_y.fpDes, nav.auto_path.pos_pid.pid_y.fpFB);
        PID_Calc(&nav.auto_path.pos_pid.pid_w, nav.auto_path.pos_pid.pid_w.fpDes, nav.auto_path.pos_pid.pid_w.fpFB);

        nav.expect_robot_global_velt.fpX = Velt_Ramp.vx + nav.auto_path.pos_pid.pid_x.fpU * 0.8f;
        nav.expect_robot_global_velt.fpY = Velt_Ramp.vy + nav.auto_path.pos_pid.pid_y.fpU * 0.8f;
        nav.expect_robot_global_velt.fpW = -0.9f+nav.auto_path.pos_pid.pid_w.fpU*0.15;//克服转向偏航力矩的前馈//nav.auto_path.pos_pid.pid_w.fpU;
        if (time_ramp >= bezier_use.total_time) // 准备上坡
        {
						nav.auto_path.pos_pid.pid_w.fpKp=2.4f;
            nav.auto_path.pos_pid.pid_x.fpSumE = 0;
            nav.auto_path.pos_pid.pid_y.fpSumE = 0;
            nav.auto_path.pos_pid.pid_w.fpSumE = 0;
