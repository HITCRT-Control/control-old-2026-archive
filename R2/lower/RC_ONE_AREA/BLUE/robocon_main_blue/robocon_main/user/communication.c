...
//总任务  inner_send[1]  0占位 1夹头 2 （空）  3对接完成  8站在地面放中层KFS  9站在R1上面取背后KFS  10站在R1放高层 11站在R1取R1KFS 12（崇武探幽恢复夹爪） 13上下400前控制机械臂姿态  14上三区改机械臂姿态 100(占位)  17R1喷气，R2可以收回机械臂  19三区重试   20初始化 200（用来解锁） 55(三区一键收腿重试让上层喷气)
//											 15 //站起来给机械臂发改姿态    
...
//板间通信通讯协议 receive
//帧头帧尾  inner_receive[0] 0xBB  inner_receive[20] 0xAA
...
//inner_receive[17] -- inner_receive[19]  红外三位数据
void usart_inner_receive(void)
{
	...
	for(uint8_t a = 0;a<inner_len;a++){
		...
		{	memcpy(&inner_receive,&uart4_receive[a],inner_len-a);
			...
			memcpy(&infra,&inner_receive[17],3);
			if(infra[0] == infra[1] && infra[1] == infra[2]){infra_data = infra[0];}else{infra_data = 0;}
