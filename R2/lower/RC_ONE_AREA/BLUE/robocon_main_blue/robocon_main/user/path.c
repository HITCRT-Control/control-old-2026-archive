...
void path_point_choose(ST_Nav *p_nav)
{
	...
	case 8://让路
		...
		Path_Point.velt[1].fpY = 0;
		Path_Point.velt[1].fpW = 0;
		Path_Point.time[0] = 600;  //ms		
	
		Path_Point.point[2].fpX = start_x - 400;
		Path_Point.point[2].fpY = start_y - 100;
		Path_Point.point[2].fpW = start_rad;
	...
	case 9://R2带着武器头重试去对接
		...
		Path_Point.point[1].fpX = DOCK_X - 650;
		Path_Point.point[1].fpY = DOCK_Y;
		Path_Point.point[1].fpW = DOCK_ANGLE;
		Path_Point.velt[1].fpX = 0;
		Path_Point.velt[1].fpY = 0;
		...
		Path_Point.point[2].fpX = DOCK_X;
		Path_Point.point[2].fpY = DOCK_Y;
		Path_Point.point[2].fpW = DOCK_ANGLE;
		Path_Point.velt[2].fpX = 0;
		Path_Point.velt[2].fpY = 0;
		Path_Point.velt[2].fpW = 0;
		Path_Point.time[1] = 600;  //ms			
...
//		Path_Point.velt[0].fpW = 0;	
//	
//		Path_Point.point[1].fpX = -3300.f;
//		Path_Point.point[1].fpY = -7370.f;
//		Path_Point.point[1].fpW =  1.55f;
//		Path_Point.velt[1].fpX = -1600;
//		Path_Point.velt[1].fpY = 0;
//		Path_Point.velt[1].fpW = 0;
//		Path_Point.time[0] = 800;  //ms	

//	
//		Path_Point.point[2].fpX = -3840.f;
//		Path_Point.point[2].fpY = -7266.f;
//		Path_Point.point[2].fpW =  1.55f;
//		Path_Point.velt[2].fpX = 0;
//		Path_Point.velt[2].fpY = 0;
//		Path_Point.velt[2].fpW = 0;
//		Path_Point.time[1] = 350;  //ms
//		
//		Path_Point.point_num = 2;		
...
//		Path_Point.velt[0].fpW = 0;	
//	
//		Path_Point.point[1].fpX = -1752.f;
//		Path_Point.point[1].fpY =  nav.auto_path.pos_pid.pid_y.fpFB;//-6755.f;
//		Path_Point.point[1].fpW =  1.55f;
//		Path_Point.velt[1].fpX = -1550;
//		Path_Point.velt[1].fpY = 0;
//		Path_Point.velt[1].fpW = 0;
//		Path_Point.time[0] = 510;  //ms	
//		
//		Path_Point.point[2].fpX = -2823;//-2863.f;//-2550;//-2616.f;
//		Path_Point.point[2].fpY = nav.auto_path.pos_pid.pid_y.fpFB;
//		Path_Point.point[2].fpW = 1.55f;
//		Path_Point.velt[2].fpX = -1550.f;
//		Path_Point.velt[2].fpY = 0;
//		Path_Point.velt[2].fpW = 0;
//		Path_Point.time[1] = 650.f;  //ms
...
//		Path_Point.point_num = 2;		
		...
		Path_Point.velt[1].fpY = 0;
		Path_Point.velt[1].fpW = 0;
		Path_Point.time[0] = 800.f;//1050.f;  //ms
...
//		Path_Point.velt[0].fpW = 0;	
//	
//		Path_Point.point[1].fpX = -840.f;
//		Path_Point.point[1].fpY = nav.auto_path.pos_pid.pid_y.fpFB;
//		Path_Point.point[1].fpW =  1.55f;
//		Path_Point.velt[1].fpX =-1550;
//		Path_Point.velt[1].fpY = 0;
//		Path_Point.velt[1].fpW = 0;
//		Path_Point.time[0] = 780;  //ms	

//	
//		Path_Point.point[2].fpX =  -2576.f;//-2863.f;//-2616.f;
//		Path_Point.point[2].fpY = nav.auto_path.pos_pid.pid_y.fpFB;
//		Path_Point.point[2].fpW = 1.55f;
//		Path_Point.velt[2].fpX = -1550.f;
//		Path_Point.velt[2].fpY = 0;
//		Path_Point.velt[2].fpW = 0;
//		Path_Point.time[1] = 980.f;  //ms
//	
...
//		Path_Point.point_num = 2;		
	...
	case 28://从曲线末尾到九宫格
		...
		Path_Point.point[1].fpX = 900 ;//-1750;//407.f;
		Path_Point.point[1].fpY =  GRID_2_Y;//-9881.f;//-9901.f;//右-9897.f;//-9926.f;
		Path_Point.point[1].fpW =	GRID_RAD;//1.8f;
...
{	
	...
	static uint16_t wait_1500 = 0;//等待1500ms计时器
	static uint8_t repeat_flag = 0;	//不断电重试标志位
	...
	switch (path_state_1)
	{
		...
		case 7://转动180度（取完矛头转过来与R1对接）并走到对接位置 需要上层机构反馈夹取成功标志位
			...
			if (vision_data_recieve.aruco_detect_flag==1||infra_data==1) {
		...
		case 8://R1二维码反馈对接成功，延迟200ms（默认打开夹爪）
			delay_change_area++;
		...
		case 9://给R1让路
			...
			if(key_receive[2]==6){repeat_flag = 1;}
			
			if (nav.nav_state == NAV_LOCK){if(repeat_flag==1){path_state_1 = 15;}else{nav_reach_state = 0; wait_1500++; if(wait_1500>=1500){wait_1500 = 0;path_state_1 = 10;}}}
			break;
		...
		case 10://走到二区入口在一区的垂直映射点
			...
			if(nav.nav_state == NAV_LOCK_POS){only_point_range_state = 0;delay_change_area = 0;path_state_1 = 14;}
		...
		case 15: //进入不断电重试阶段
			if(key_receive[1]==1){path_state_1 = 10;}
			break;
			
		case 16: //一区带着武器头重试
			if(!nav_inited){nav.auto_path.number_point = 9;nav.nav_state = NAV_POINT_TO_POINT; flag_area_to_point = 2;nav_inited = 1;nav_reach_state = 3;}
		...
		switch(vision_data_recieve.path_number)//根据路径判断机械臂 高 中 低
				{
					...
					case 2:
				if(vision_data_recieve.state_2==2||vision_data_recieve.state_2==4){inner_send[2] = 1;}//左侧
				else{inner_send[2] = 2;}
						break;
...
{		
    ...
    static uint16_t qrcode_timer = 0;//等待二维码时间，超出1000ms还没识别那么直接走,默认放到2列
	  static uint8_t flag_first = 0;
		...
		static uint8_t flag_R1_POS = 1;  //R1在R2的哪一侧
	...
	if(vision_data_recieve.aruco_detect_flag==2&&vision_data_recieve.arcuo_left_or_right==1){flag_qrcode=1;}
	else if(vision_data_recieve.aruco_detect_flag==3){flag_qrcode=2;}
	else if(vision_data_recieve.aruco_detect_flag==4&&vision_data_recieve.arcuo_left_or_right==2){flag_qrcode=3;}
	
	if(vision_data_recieve.aruco_detect_flag==10||infra_data==21){force_interrupt = 1;}  //强制打断的二维码，后面将切到R1控制模式
...
//							qrcode_timer++;
							if(flag_qrcode==1){path_state_3 = 2;}
							else if(flag_qrcode==2){path_state_3 = 3;}
							else if(flag_qrcode==3){path_state_3 = 4;}			
//							if(qrcode_timer>=1000){flag_qrcode=2;path_state_3 = 3;}	//路上没有识别到，等待1000ms也没识别到就默认放中间
		...
		case 9://取R1
	if((vision_data_recieve.aruco_detect_flag == 6||infra_data==13)&&flag_get==0){inner_send[1] = 17;} //可以拿走
	...
	if(flag_get){timer_delay++; if(timer_delay>=5){path_state_3 = 10;} }
		...
		case 10://等待R1信号 非打断
			if(vision_data_recieve.aruco_detect_flag == 7||infra_data==14){inner_send[1] = 10;}
			if(inner_receive[3] ==2 ){inner_send[1] = 200;path_state_3 = 11;}
			
			break;
		
		case 11: //等待R1信号再取一个R1KFS
			if(vision_data_recieve.aruco_detect_flag == 8||infra_data==15){inner_send[1] = 11;path_state_3 = 9;}
			break;
...
//    if(!nav_inited){nav.nav_state = NAV_LOCK;nav_inited = 1;}
    ...
    if(flag_qrcode != pre_flag_qrcode){
    ...
    else if(vision_data_recieve.aruco_detect_flag == 7||infra_data==14){
		...
		if(flag_get==1)
		{
			 ...
			 else if(vision_data_recieve.aruco_detect_flag == 7||infra_data==14){flag_put_cnt++; inner_send[1] = 8; path_state_3 = 15; }
...
void all_path_logic(void)
{
	switch (all_path_state)
	{
		...
		case 4://无法大胜，在一区重试
//		if(first_path_state==0){path_state_4 = 1;first_path_state = 1;}			
//		path_4();
			break;
		
		case 5://三区导航重试 拿一个KFS
		...
		case 7://一区夹着武器头重试跑到对接点位
		if(first_path_state==0){path_state_1 = 16;first_path_state = 1;}			
		path_1();					
			break;		
...
{	
	...
	else if(key_receive[2]==6&&first_flag==0) {all_start_state = 7;inner_send[1] = 56;first_flag = 1;}
	...
	if(all_start_flag==1)
	{
		switch(all_start_state)
		{
			...
			case 5://收腿解锁底盘
				...
				if(up_down_state==0){inner_send[1] = 55;}
				break;
